# TCO VM Refactoring Plan: Option A - Separate Local Stack

## Overview

Based on expert guidance, implementing **Option A (Separate Local Stack)** is the correct solution for enabling TCO. This document provides the detailed implementation plan.

## Current Problem

The current VM design has a fundamental mismatch:
- **Arguments** are stored in `frame.locals` after being copied from the stack
- **SetLocal** writes to `frame.locals` but expects to pop from the stack
- **TailCall** reuses the frame and overwrites `frame.locals`
- After `Call` truncates the stack, `SetLocal` has nothing to pop
...
> **"Arguments are just initialized local variables. The expression stack is temporary workspace that should be empty on function entry and contain exactly one value on exit."**

## Implementation Steps

### Phase 1: CallFrame Structure Modification

#### 1.1 Update CallFrame in `physics_world/src/vm/call_frame.rs`
...
#### 1.2 Update VmState to track value_stack separately

```rust
pub struct VmState {
    /// The shared value stack for expression evaluation
    pub value_stack: Vec<Value>,
    /// Call stack containing frames with local variables
...
#### 2.3 Push - Push to value_stack (no change)

```rust
pub fn handle_push(vm: &mut VmState, value: Value) -> Result<(), VmError> {
    vm.value_stack.push(value);
    Ok(())
}
```

#### 2.4 Pop - Pop from value_stack (no change)
...
pub fn handle_return(vm: &mut VmState) -> Result<(), VmError> {
    ...
    // If there are frames below, push return value to value_stack
    // Note: We don't need to worry about frame.locals persisting
    // because locals are tied to their frame
    if let Some(caller_frame) = vm.call_stack.last_mut() {
        // Push return value - caller will use GetLocal if needed
        vm.value_stack.push(return_value);
    } else {
        // No caller frame - this is the top level
        vm.value_stack.push(return_value);
    }
...
pub fn handle_make_closure(vm: &mut VmState, arg_count: u16) -> Result<(), VmError> {
    ...
    // Last value is the arg_count
    // Second to last is the code pointer
    // Remaining are environment values
...
1. `physics_world/src/vm/call_frame.rs` - Add locals Vec to CallFrame
