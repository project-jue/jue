# **Project Jue: Physics Layer Specification v1.0**

## **1. Overview & Guarantees**

The Physics Layer is a **deterministic, introspectable, single-threaded virtual machine** that provides the computational substrate for Dan-World actors. It enforces the Assumption of Insufficient Knowledge and Resources (AIKR) through precise resource limits.
...
The Physics Layer runs a **single OS thread**. Concurrency is cooperative via the `Yield` opcode.
...
impl PhysicsScheduler {
    ...
    pub fn tick(&mut self) -> Result<(), PhysicsLayerError> {
...
## **7. Physics Layer External API**

The clean interface that Jue-World will call. This is the **only public API** of the Physics Layer.
...
4.  **Unit 4: Scheduler & API (`scheduler.rs`, `api.rs`)**
    *   Implement the round-robin `PhysicsScheduler` and the public `PhysicsLayer` API.
