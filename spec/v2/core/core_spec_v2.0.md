# **Project Jue: Core-World Specification v2.0**
## **The Immutable Formal Kernel**
...
**V2 Change:** Core-World explicitly does not reason about capabilities. Capabilities are operational constructs belonging to Physics-World and Jue-World. Core-World's job is to verify that capability-wrapped operations preserve the underlying λ-calculus semantics.
...
**V2 Optimization:** Implement efficient normalization using explicit stacks to handle deeply nested terms without recursion limits.
...
*   **Key Principle:** The verifier does **not** take a target term as input. It inspects the proof object alone to determine what equivalence it proves.
...
### **6. Serialization Format (V2 Addition)**

**V2 Standard:** CoreExpr and Proof must be serializable to a standardized binary format for cross-layer communication.
...
// - Proof variants: Tagged union with explicit length prefixes
...
**Rationale:** Standardized serialization ensures that Jue-World and Physics-World can consistently exchange Core terms and proofs, enabling cross-layer verification.

### **7. Relationship to Capabilities (V2 Clarification)**

**Core-World does not reason about capabilities.** Capabilities are operational constructs that belong to Physics-World and Jue-World. However, Core-World plays a crucial role:

1. **Capability Safety:** When Jue-World compiles code that uses capabilities, it must prove that the capability-wrapped operations preserve the underlying λ-calculus semantics.
2. **Proof Composition:** A proof that code with capabilities is correct can be composed from:
   - Core-World proofs about the pure λ-calculus fragments
   - Physics-World guarantees about capability enforcement
3. **Boundary:** The boundary between pure computation and capability-mediated operations must be explicit in proofs.

**Example:** If Jue code uses `IoReadSensor` capability, the proof obligation is:
- The sensor read operation returns a value of type `Int`
- The surrounding λ-calculus code correctly handles that `Int`
- Core-World verifies the λ-calculus part; Physics-World verifies the capability part
...
### **9. V2 Migration Notes**

**From V1.0 to V2.0:**
- No breaking changes to CoreExpr or Proof structures
- Add serialization functions (required for V2)
- Add explicit documentation about capability boundary
- Optimize normalization algorithm for large terms

**Backward Compatibility:** V2 is fully backward compatible with V1.0 proofs and terms. The additions are extensions, not modifications.

This specification defines Core-World v2.0. Implementation is a direct translation of these rules into code, with the addition of standardized serialization for cross-layer communication.